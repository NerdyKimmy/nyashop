<?php

namespace App\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;

class ProfileRequest extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, mixed>
     */
    public function rules()
    {
        return [
            'first_name' => 'required|string|max:255',
            'last_name'  => 'required|string|max:255',
            'phone'      => ['required', 'regex:/^\+?[0-9]{10,15}$/'],

            'email'      => 'required|email|max:255|unique:users,email,' . auth()->id(),

            'billing.address1'     => 'nullable|string|max:255',
            'billing.address2'     => 'nullable|string|max:255',
            'billing.city'         => ['required', 'regex:/^[a-zA-Z\s\-]+$/', 'max:100'],
            'billing.state'        => 'nullable|string|max:100',
            'billing.country_code' => 'required|string|size:3',
            'billing.zipcode'      => ['required', 'regex:/^[0-9]{4,10}$/'],

            'shipping.address1'     => 'nullable|string|max:255',
            'shipping.address2'     => 'nullable|string|max:255',
            'shipping.city'         => ['required', 'regex:/^[a-zA-Z\s\-]+$/', 'max:100'],
            'shipping.state'        => 'nullable|string|max:100',
            'shipping.country_code' => 'required|string|size:3',
            'shipping.zipcode'      => ['required', 'regex:/^[0-9]{4,10}$/'],

        ];
    }

    public function attributes()
    {
        return [
            'billing.address1' => 'address 1',
            'billing.address2' => 'address 2',
            'billing.city' => 'city',
            'billing.state' => 'state',
            'billing.zipcode' => 'zip code',
            'billing.country_code' => 'country',
            'shipping.address1' => 'address 1',
            'shipping.address2' => 'address 2',
            'shipping.city' => 'city',
            'shipping.state' => 'state',
            'shipping.zipcode' => 'zip code',
            'shipping.country_code' => 'country',
        ];
    }
}
